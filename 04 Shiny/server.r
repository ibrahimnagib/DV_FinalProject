...
shinyServer(function(input, output) {
  ...
  df1 <- eventReactive(input$clicks4, { data.frame(fromJSON(getURL(URLencode(gsub("\n", " ", 'skipper.cs.utexas.edu:5001/rest/native/?query="Select VEHICLE_MANUFACTURER_NAME, RATED_HORSEPOWER
                from CARS
                group by VEHICLE_MANUFACTURER_NAME, RATED_HORSEPOWER
                order by Avg(RATED_HORSEPOWER); ')), httpheader=c(DB='jdbc:oracle:thin:@sayonara.microlab.cs.utexas.edu:1521:orcl', USER='C##cs329e_in2422', PASS='orcl_in2422', MODE='native_mode', MODEL='model', returnDimensions = 'False', returnFor = 'JSON', p1=Rated_Horsepower_value()), verbose = TRUE)))
  })
  
  output$distPlot4 <- renderPlot({ 
    plot4<- ggplot(car, aes(x = reorder(VEHICLE_MANUFACTURER_NAME, RATED_HORSEPOWER, FUN=median), y = RATED_HORSEPOWER)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x="Vehicle Manufacturer Name", y="Rated Horsepower")
    
    plot4
  }) 
  
  df1 <- eventReactive(input$clicks5, { data.frame(fromJSON(getURL(URLencode(gsub("\n", " ", 'skipper.cs.utexas.edu:5001/rest/native/?query="Select VEHICLE_MANUFACTURER_NAME, RATED_HORSEPOWER, CO2_G_MI_
          from CARS;')), httpheader=c(DB='jdbc:oracle:thin:@sayonara.microlab.cs.utexas.edu:1521:orcl', USER='C##cs329e_in2422', PASS='orcl_in2422', MODE='native_mode', MODEL='model', returnDimensions = 'False', returnFor = 'JSON', p1=Rated_Horsepower_value()), verbose = TRUE)))
  })
  
  output$distPlot5 <- renderPlot({ 
    plot4<- ggplot(car, aes(x=RATED_HORSEPOWER)) + geom_histogram(aes(fill = ..count..),binwidth=35.4) + labs(x="Rated Horsepower", y="Count")
    
    plot4
  }) 
  
  
  df1 <- eventReactive(input$clicks6, { data.frame(fromJSON(getURL(URLencode(gsub("\n", " ", 'skipper.cs.utexas.edu:5001/rest/native/?query="select Rated_Horsepower,Co2_G_MI_,Vehicle_Type from CARS where CO2_G_MI_ is not null and RATED_HORSEPOWER <"p1" ')), httpheader=c(DB='jdbc:oracle:thin:@sayonara.microlab.cs.utexas.edu:1521:orcl', USER='C##cs329e_in2422', PASS='orcl_in2422', MODE='native_mode', MODEL='model', returnDimensions = 'False', returnFor = 'JSON', p1=Rated_Horsepower_value()), verbose = TRUE)))
  })
