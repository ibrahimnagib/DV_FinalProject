...
#### Shiny App:
We used Shiny, to build an app to demonstrate our more interesting visualizations in a neat dashboard.

#### Data:
The data set we chose to analyze and visualize is a data set of tested vehicles. A group of 2016 vehicles were tested for various measures including horsepower, carbon emmissions, nitrogen emmisions, engine displacement, and various other measures. We used a modified R_ETL.R file to ...

# ```{r}
# source("../01 Data/R_ETL_car.R", echo = FALSE)
# ```

#### Our Pre-Analysis hypothesis:
Before starting our analysis, we first discussed what we expected to see, and what we wanted the visualizations to help prove or support. We expect some positive correlation between horsepower, carbon dioxide emmissions, and N2O emmissions ie. as horsepower increases, we expect to see an overall increase in co2, and n2o, we also expect to see a strong positive correlation between egnine displacement, horsepower, and fuel economy, naturally, we believe a larger engine will produce more horsepower, and be less fuel efficient.
